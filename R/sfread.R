...
#' First: its default behaviour is to read in large integers as 'numeric' rather than
#' 'integer64' (in contrast to fread{data.table}). At this point in time, much of R
#' doesn't seem to be yet ready to handle the integer64 class correctly. The call
#' data.matrix(aDataFrameWithInt64) doesn't convert correctly. The Architect data
#' viewer doesn't display integer64 correctly.
#' The second feature of this function is that it caters adequately for
#' files with sparse columns of data where the type is hard to
#' auto-guess, and where those columns do not always exist in the dataset.
